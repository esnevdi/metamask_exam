...
$(document).ready(function() {
    const productRegistryContractAddress = '0x36738092C4fc369D59E4C6374BdE836a8192f6ec';
    ...
    $('#linkHome').click(function() { showView("viewHome") });						//Home 누르면 홈 열리고
    $('#linkSubmitDocument').click(function() { showView("viewSubmitDocument"); showTable();  });	//버튼 클릭했을 때 어떤 뷰가 나오게 해라.
	...
	async function showTable() {
		...
		if (window.ethereum)		//현재 메타마스크가 설치되어 있느냐? 를 묻는 문법
			try {
				await window.ethereum.enable();	//설치되어 있으면 메타마스크 enable 시켜
			} catch (err) {
                return showError("Access to your Ethereum account rejected.");
			}
		if (typeof web3 === 'undefined')		//설치 안 되어 있으면 설치해달라 메세지 출력
    ...
    async function itemUploadButton() {
		...
		//수량, 품목, 지역 을 smart contract에 보내서 저장시킴
